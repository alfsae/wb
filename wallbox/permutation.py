...
def min_quantity_of_permutations(*sequence: Union[bool, int]) -> int:
    ...
    For example, given the sequence 0,1,1,0 how many changes are needed so that the result is 0,1,0,1
    ...
    [0, 1, 0, ...] and [1, 0, 1, ...]
    We don't need to create those arrays. Iterating over the sequence and knowning the value and the index 
    we can compare it with both expected ideal values in the same position  
    """
